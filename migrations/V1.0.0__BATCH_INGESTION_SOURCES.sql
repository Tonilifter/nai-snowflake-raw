...
create or replace table TB_BATCH_CONFIG (
    ...
    TX_SOURCE_REGEX varchar not null comment 'Regular expression of the sources to be loaded on the target table',
    ...
    CO_EXTERNAL_STAGE varchar not null comment 'Stage in which the source with the information is allocated',
    TX_FILE_SEPARATOR varchar default '' comment 'Column separator for this file. Specific for batch files',
...
create or replace table TB_BATCH_FIELDS_CONFIG (
    ...
    DS_FIELD varchar not null comment 'Field that will be parsed',
