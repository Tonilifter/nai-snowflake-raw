...
CREATE OR REPLACE TASK TSK_CHECK_NEW_DMP_DM3_GLUE_Z001 WAREHOUSE = {{warehouse}} SCHEDULE = 'USING CRON 0 * * * * UTC' AS 
    call DB_INGESTION_TOOLS_{{environment}}.STREAMING.CHECK_NEW_STREAMING_TABLE('DB_LND_{{environment}}', 'SAP_DMP_DM3_GLUE', 'Z001_', '_CONSOLIDATED', TRUE, TRUE);
CREATE OR REPLACE TASK TSK_CHECK_NEW_DMP_DM3_GLUE_Z003 WAREHOUSE = {{warehouse}} SCHEDULE = 'USING CRON 0 * * * * UTC' AS 
    call DB_INGESTION_TOOLS_{{environment}}.STREAMING.CHECK_NEW_STREAMING_TABLE('DB_LND_{{environment}}', 'SAP_DMP_DM3_GLUE', 'Z003_', '_CONSOLIDATED', TRUE, TRUE);
